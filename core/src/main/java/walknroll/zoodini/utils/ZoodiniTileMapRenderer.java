...
public class ZoodiniTileMapRenderer extends OrthogonalTiledMapRenderer {

    public ZoodiniTileMapRenderer(TiledMap map, Batch batch) {
        super(map, batch);
        TiledMapTileLayer layer = (TiledMapTileLayer) map.getLayers().get(0);
        DebugPrinter.println("Height: " + layer.getHeight());
        DebugPrinter.println("Width: " + layer.getWidth());

    }

    @Override
    public void renderTileLayer (TiledMapTileLayer layer) {
        DebugPrinter.println("Rendering layer: " + layer.getName());
        ...
        for (int row = row2; row >= row1; row--) {
            ...
            for (int col = col1; col < col2; col++) {
                ...
                if (cell == null) {
//                    DebugPrinter.println("Row, col" + row + ", " + col + " is null");
                    x += layerTileWidth;
                    continue;
                }

//                DebugPrinter.println(" has a cell");
    ...
    public void renderTileLayerRow (TiledMapTileLayer layer, int row) {
        final Color batchColor = batch.getColor();
        final float color = Color.toFloatBits(batchColor.r, batchColor.g, batchColor.b, batchColor.a * layer.getOpacity());
        ...
        float y = row * layerTileHeight + layerOffsetY;
        float xStart = col1 * layerTileWidth + layerOffsetX;
        final float[] vertices = this.vertices;
